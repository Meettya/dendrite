...
(function() {
  ...
  dependencies = {"Z1kxgSP":{"..":"Z2eESlk"},"Z2eESlk":{"lodash":"Zc7rvy"}};
...
"Z1kxgSP": function(exports, module, require) {
...
"Z2eESlk": function(exports, module, require) {
  ...
  **GitHub repository** [dendrite](https://github.com/Meettya/dendrite)
  ...
  module.exports = Dendrite = (function() {
    ...
    function Dendrite(options) {
      ...
      this._publishFiring = bind(this._publishFiring, this);
      this._unsubscribeResume = bind(this._unsubscribeResume, this);
      this._publisher = bind(this._publisher, this);
      this._publisherEngine = bind(this._publisherEngine, this);
      this._parseVerboseLevel = bind(this._parseVerboseLevel, this);
      this._getNextTaskNumber = bind(this._getNextTaskNumber, this);
      this._publishingDec = bind(this._publishingDec, this);
      this._publishingInc = bind(this._publishingInc, this);
      this._isPublishing = bind(this._isPublishing, this);
      this._isInternalChannel = bind(this._isInternalChannel, this);
      this.off = bind(this.off, this);
      this.on = bind(this.on, this);
      this.isTopicListened = bind(this.isTopicListened, this);
      this.getListenedTopicsList = bind(this.getListenedTopicsList, this);
      this.publishAsync = bind(this.publishAsync, this);
      this.publishSync = bind(this.publishSync, this);
      this.publish = bind(this.publish, this);
      this.unsubscribe = bind(this.unsubscribe, this);
      this.subscribeGuarded = bind(this.subscribeGuarded, this);
    ...
    Dendrite.prototype.subscribeGuarded = function(topics, callback, watchdog, context) {
      var base, i, len, ref, task_number, topic;
      if (context == null) {
        context = {};
      }
      ...
      ref = this._topicsToArraySplitter(topics);
      for (i = 0, len = ref.length; i < len; i++) {
        topic = ref[i];
        (base = this._subscriptions_)[topic] || (base[topic] = []);
    ...
    Dendrite.prototype.unsubscribe = function(topics, callback, context) {
      var i, idx, j, len, len1, ref, ref1, ref2, task, task_number, topic;
      if (topics.topics) {
        ref = this._handlerParser(topics, callback, context), topics = ref[0], callback = ref[1], context = ref[2];
      }
      context || (context = {});
       ...
       */
      ref1 = this._topicsToArraySplitter(topics);
      for (i = 0, len = ref1.length; i < len; i++) {
        topic = ref1[i];
        if (!(this._subscriptions_[topic] != null)) {
          continue;
        }
        if (_.isFunction(callback)) {
          ref2 = this._subscriptions_[topic];
          for (idx = j = 0, len1 = ref2.length; j < len1; idx = ++j) {
            task_number = ref2[idx];
    ...
    Dendrite.prototype.publish = function() {
      var data, topics;
      topics = arguments[0], data = 2 <= arguments.length ? slice.call(arguments, 1) : [];
    ...
    Dendrite.prototype.getListenedTopicsList = function() {
      var listiners, ref, results, topic;
      ref = this._subscriptions_;
      results = [];
      for (topic in ref) {
        listiners = ref[topic];
    ...
    Dendrite.prototype.isTopicListened = function(topic) {
      ...
      return !!((ref = this._subscriptions_) != null ? (ref1 = ref[topic]) != null ? ref1.length : void 0 : void 0);
    };
    ...
    Dendrite.prototype.off = function(topic) {
      var callback, context, lc_topic, ref;
      if (topic != null ? topic.topics : void 0) {
        ref = this._handlerParser(topic), topic = ref[0], callback = ref[1], context = ref[2];
      } else {
    ...
    Dendrite.prototype._publisher = function(type, topics, data) {
      var _publish, _unsubscribe, i, j, len, len1, ref, ref1, ref2, task_number, topic;
      if (!_.isString(topics)) {
        throw this._publishErrorMessage(topics, data);
      }
      ref = this._publisherEngine(type), _publish = ref[0], _unsubscribe = ref[1];
      ref1 = this._topicsToArraySplitter(topics, false);
      for (i = 0, len = ref1.length; i < len; i++) {
        topic = ref1[i];
    ...
    Dendrite.prototype._topicsToArraySplitter = function(topics, skip_duplicate) {
      var i, len, ref, results, topic, used_topics;
    ...
    Dendrite.prototype._unsubscribeResume = function() {
      ...
      while (task = typeof (base = this._unsubscribe_queue_).shift === "function" ? base.shift() : void 0) {
        if (this._observer_verbose_level_ >= DEBUG) {
          if (typeof console !== "undefined" && console !== null) {
    ...
    Dendrite.prototype._publishFiring = function(topic, task, data) {
      ...
      try {
        ...
        if (this._observer_verbose_level_ >= ERROR) {
          if (typeof console !== "undefined" && console !== null) {
            console.error("Error: on call subscribe callback we got exception\n  topic     = |" + topic + "|\n  callback  = |" + task[0] + "|\n  watchdog  = |" + task[2] + "|\n  object    = |" + ((typeof JSON !== "undefined" && JSON !== null ? typeof JSON.stringify === "function" ? JSON.stringify(task[1]) : void 0 : void 0) || task[1]) + "|\n  data      = |" + (data != null ? data.join(', ') : void 0) + "|\n  error     = |" + err + "|");
          }
        }
        if (this._observer_verbose_level_ === DEBUG && (err.stack != null)) {
          if (typeof console !== "undefined" && console !== null) {
